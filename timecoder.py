...
# Finishes changing Timecode to frames.  Additional math is needed to address frame loss.
def drop_tc_to_frame(frames, seconds, minutes, hours, fps):
    ...
    drop_frame = ((hour_frames * hours) + (minute_frames * minutes) + (fps * seconds) + frames) - (
                dropped_frames * (total_minutes - (total_minutes // 10)))
...
def drop_frame_tc(frame_in, fps):
    ...
    if ten_min_counter > drop_frames:
        frame_number = frame_number + (drop_frames * 9 * ten_min_counter) + drop_frames * (
                    (ten_min_counter_mod - drop_frames) // fps)
    else:
